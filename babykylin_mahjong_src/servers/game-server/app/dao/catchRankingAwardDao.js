...
dao.getByPlayerId = function (playerId, cb) {
    var sql = 'SELECT rank,drew FROM CatchRankingAward WHERE playerId = ?',
        args = [playerId];
...
function clear(cb) {
    var sql = 'DELETE FROM CatchRankingAward WHERE 1';
...
function batchAdd(awardData, cb) {
    var sql = 'INSERT INTO CatchRankingAward(rank,playerId) VALUES ?',
        args = [];
    awardData = awardData || [];
...
dao.setDrew = function (playerId, cb) {
    var sql = 'UPDATE CatchRankingAward SET drew = 1 WHERE playerId = ? AND drew = 0',
        args = [playerId];
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (err) {
