...
dao.load = function (type,cb) {
    var sql = 'SELECT playerId,score FROM rankList WHERE type = ?',
        args = [type];
 ...
 *   查询总榜上玩家的其他信息
 * */
dao.getPlayerRankingInfo = function (playerId,cb) {
    var sql = 'SELECT P.id, P.playername,P.headPicId,H.posInfo FROM player P INNER JOIN heroBag H ON P.id = H.playerId AND' +
        ' P.curHeroPos = H.pos WHERE P.id = ?';
    // playerIds = playerIds || [];
    // if (playerIds.length <= 0) {
    //     return utils.invokeCallback(cb, null, []);
    // }
    pomelo.app.get('dbclient').query(sql, [playerId], function (err, rec) {
        if (err) {
            ...
            utils.invokeCallback(cb, null, rec[0]);
        }
    });
};
