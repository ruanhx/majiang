...
pro.reset = function () {
    ...
    _.each(playerIds, function (playerId) {
        ...
        if (!!player) {
            player.buffManager.resetBuyCnt();
            player.occasionManager.resetDailyCnt();
            player.resetEndlessBoxToHeroCnt(new Date());
        }
    });
};

pro.resetWeekHighScore = function(){
    logger.debug('reset time = %s', new Date().toTimeString());
    // 即时重置在线部分玩家，不在线的玩家等上线后再进行重置
    var playerIds = area.getPlayerIds();
    _.each(playerIds, function (playerId) {
        var player = area.getPlayer(playerId);
        if (!!player) {
            player.setLastWeekHighScore();
            player.weekHighScoreMgr.reset();
        }
    });
};
