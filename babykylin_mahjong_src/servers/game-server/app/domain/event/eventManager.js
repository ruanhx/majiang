...
function addSaveEvent(player) {
    ...
    player.on('saveHeroHistory',function(vData){
        app.get('sync').exec('heroHistorySync.update', [player.id].join('_'), vData, player.id);
    });
    player.on('updateActivityEctype', function (newRec) {
        app.get('sync').exec('passedActivityEctypeSync.updatePassedInfo', [player.id, newRec.activityid].join('_'), newRec, player.id);
    });
    player.on('saveEquipHistory',function(vData){
        app.get('sync').exec('equipHistorySync.update', [player.id].join('_'), vData, player.id);
    });
    player.on('saveDivisionPerson',function(vData){
        app.get('sync').exec('divisionPersonSync.update', [player.id].join('_'), vData, player.id);
    });
    player.on('saveFriendPerson',function(vData){
        app.get('sync').exec('friendPersonSync.update', [player.id].join('_'), vData, player.id);
    });
    player.on('savePlayerRefresh',function(vData){
        app.get('sync').exec('playerRefreshSync.save', player.id, vData, player.id);
    });
    player.on('assistFightSync',function(vData){
        app.get('sync').exec('assistFightSync.save', player.id, vData, player.id);
    });
    player.on('saveTrain',function(vData){
        app.get('sync').exec('trainSync.save', player.id, vData, player.id);
    });
    player.on('saveCatchTreasure',function(vData){
        app.get('sync').exec('catchTreasureSync.save', player.id, vData, player.id);
    });
    player.on('removeMail',function(vData){
        app.get('sync').exec('mailSync.remove', [player.id, vData.id].join('_'), vData, player.id);
    });
    player.on('saveMail',function(vData){
        app.get('sync').exec('mailSync.save', [player.id, vData.id].join('_'), vData,player.id);
    });
    player.on('saveEndlessPVPBox',function(vData){
        app.get('sync').exec('endlessPVPBoxSync.save', player.id, vData,player.id);
    });
    player.on('saveBarrierPromote',function(vData){
        app.get('sync').exec('barrierPromoteSync.save', [player.id, vData.chapterId].join('_'), vData,player.id);
    });
}
