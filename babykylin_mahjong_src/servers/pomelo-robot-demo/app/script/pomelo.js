...
module.exports.createInstance = function() {
    ...
    pomelo.request = function (route, msg, cb) {
        ...
        reqId++;
        // 注意:原始的pomelo-robot-demo 使用的pomelo-protocol版本是0.1.0,而当前服务器端使用的版本是0.1.2,导致了封包解析出错，部分请求包由于解析回应包时
        // msg.id解析错误，导致没有回应包
//  console.log('request reqId = %s, route = %s', reqId, route);
...
//    console.log('onData reqId = %s', msg.id);
            delete routeMap[msg.id];
            if (!msg.route) {
//        console.log('onData ERROR reqId = %s not found!', msg.id);
                return;
            }
        }
