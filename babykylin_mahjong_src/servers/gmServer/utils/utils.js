...
utils.getServerDay = function(app){
    var serverStartTime = new Date(),
        curDate = new Date();
    serverStartTime.setTime(app.get('serverStartTick'/*'serverStartTime'*/));
 ...
 * 获取下一天某个整点时间
 */
utils.getNextDayXHour = function(hour){
    var now = new Date();
    now.setHours(hour||0,0,0,0);
    return now.getTime()+utils.getOneDaySesc();
}
 ...
 * @returns {String}
 *
 * var str='这是一个测试的字符串：{0} {1}'.format('Hello','world');或:var str='这是一个测试的字符串：{str0} {str1}'.format({str0:'Hello',str1:'world'});
 */
