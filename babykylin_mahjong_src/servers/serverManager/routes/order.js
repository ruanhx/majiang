...
var dbClient = require('../dao/mysql/mysql'),
    dbClient4Log = require('../dao/mysql4Log/mysql'),
...
function invokeGameServerCb(platform, ip, port, msg, cb){
    ...
    var rechargeCfg = dataApi.rechargeCfg.findBy('productId', parseInt(msg.productId));

    var rechargeData = null;
    if (rechargeCfg && rechargeCfg.length>=1) {
        rechargeData = rechargeCfg[0];
    }
    ...
    request({url: url, qs: qs}, function(err, response, body){
        ...
        if(rechargeData){
            rechargeDao.getRechargeInfoByMAC(dbClient,msg.uid,function (err,res) {
                var money = 0;
                var diamond = 0;
                console.log('invokeGameServerCb rechargeData %j res:%j', rechargeData,res);
                if(!err){
                    if(res){
                        money = res.money + rechargeData.price;
                        diamond = res.diamond + rechargeData.diamond;
                    }else {
                        money = rechargeData.price;
                        diamond = rechargeData.diamond;
                    }
                    rechargeDao.pushRechargeInfo(dbClient,{MAC:msg.uid,money:money,diamond:diamond},function (err,res) {
                        console.info('rechargeDao.pushRechargeInfo success');
                        logDao.writeLogPayRec(dbClient,{orderId:msg.orderId,channelId:msg.channel,accountId:msg.uid,amount:money,status:1,addtime:Date.now(),serverId:msg.serverId},function(){});
                    });
                }
            });
...
//泰奇-互动-国内
exp.order_taiqi_inland_android = function(req,res){
    var platformName = 'taiqi_inland_android';
    var funName = 'order_taiqi_inland_android';
