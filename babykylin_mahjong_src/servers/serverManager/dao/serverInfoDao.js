...
serverInfoDao.getServerList = function (dbClient, cb) {
    var sql = 'SELECT serverId as ID,name,IP,port,uptime,clientVersion,resVersion,packages,pkgUrl,alias,clientMinVer,flag,tips ,doMainName,gmPort,isSkipGuide FROM serverinfo order by ID desc'
        , args = [];
...
serverInfoDao.getServerInfoByID = function (dbClient, serverId, cb) {
    var sql = 'SELECT serverId as ID, name, IP FROM serverinfo WHERE serverId = ?',
        args = [serverId];
    dbClient.query(sql, args, function (err, servers) {
        if (!!err) {
...
serverInfoDao.pushServerInfo = function (dbClient, serverInfo, cb) {
    var sql = 'INSERT INTO serverinfo(serverId,name,IP,port,startTime,uptime,onlineCnt,maxOnlineCnt,clientVersion,resVersion,' +
            'packages,pkgUrl,alias,clientMinVer,flag,tips,doMainName,gmPort,isSkipGuide) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE' +
        ...
        'pkgUrl=VALUES(pkgUrl),clientMinVer=VALUES(clientMinVer),flag=VALUES(flag),tips=VALUES(tips),doMainName=VALUES(doMainName),gmPort=VALUES(gmPort),isSkipGuide=VALUES(isSkipGuide)',
        now = Date.now(),
        args = [serverInfo.serverId,serverInfo.name, serverInfo.ip, serverInfo.port, now, now, serverInfo.onlineCnt, serverInfo.maxOnlineCnt,
            serverInfo.clientVersion, serverInfo.resVersion, serverInfo.packages, serverInfo.pkgUrl, serverInfo.alias,
            serverInfo.clientMinVer, serverInfo.flag, serverInfo.tips,serverInfo.doMainName,serverInfo.gmPort,serverInfo.isSkipGuide];
